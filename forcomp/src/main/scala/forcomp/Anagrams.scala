...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def temp(x: Occurrences, y: Occurrences, acc: Occurrences): Occurrences = (x, y) match {
      case (x, Nil) => acc ::: x
      case (Nil, y) => acc ::: y
      case (xx :: xs, yy :: ys) => if (xx._1 < yy._1) temp(xs, y, acc :+ xx)
            else temp(xs, ys, acc :+ (xx._1, xx._2 - yy._2))
    }
    temp(x, y, List()).filter(_._2 != 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def temp(occ: Occurrences): List[Sentence] = occ match {
      case Nil => List(List())
      case occ =>
        for {
          subOcc <- combinations(occ)
          word <- dictionaryByOccurrences(subOcc)
          sentence <- temp(subtract(occ, subOcc))
        } yield word :: sentence
    }
    temp(sentenceOccurrences(sentence))
  }  
}

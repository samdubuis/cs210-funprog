...
// Evaluation -----------------------------------
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    case 'def :: (Symbol(name) :: params) :: body :: expr :: Nil =>
      if (env == globalEnv)
        globalEnv = env.extendRec(name, env1 => mkLambda(asList(params) map paramName, body, env1))
        eval(expr, env.extendRec(name, env1 => mkLambda(asList(params) map paramName, body, env1)))
    case 'case :: expr_scrut :: expr_1toN =>
      eval(mkCase(expr_scrut, expr_1toN), env) 
    case 'quote :: y :: Nil =>
      y
  ...
  def mkCase(scrut: Data, expr: List[Data]): Data = expr match {
    case (x :: y :: Nil) :: Nil => y
    case (x :: y :: Nil) :: xs =>
      List('if, List('=, scrut, x), y, mkCase(scrut, xs))
    case _ => sys.error("Case error")
  }

...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      val nleft = left.filterAcc(p, acc)
      val nright = right.filterAcc(p, acc)
      val lr = nleft union nright
    ...
    def mostRetweeted: Tweet = {
      val allTweets = right.union(left);
      val morePopular = allTweets.filter(p => p.retweets > elem.retweets)
